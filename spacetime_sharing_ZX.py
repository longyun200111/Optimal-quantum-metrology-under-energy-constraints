'''
Calculate the optimal precision for the global spacetime-sharing battery
...
def solve_accurate():
    ...
    data = []
    opt_val = {
        E: min([sol for sol in res if sol['energy'] == E], key=lambda x: x['val'])
        for E in E_val
    }
    ...
    try:
        ...
        for row in data:
            ...
            if row.get(col_name, None) is not None:
                if opt_val[E]['val'] < row[col_name]:
                    row[col_name] = opt_val[E]['val']
            else:
                row[col_name] = opt_val[E]['val']
    except:
        header = ['E', 'val_shared']
