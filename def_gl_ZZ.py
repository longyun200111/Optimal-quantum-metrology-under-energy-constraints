...
def calc_superposition(args, dim=2):
    ...
    for i in range(2):
        lambda_x = np.exp(-1j * np.angle(mul_val[i].value))
        lambda_x_real[i].value = np.real(lambda_x)
        lambda_x_imag[i].value = np.imag(lambda_x)
...
def solve_accurate():
    ...
    for arg in args:
        task.append(
            pool.apply_async(calc_superposition, args=(arg,), callback=update)
        )
    for t in task:
